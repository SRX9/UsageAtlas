...
const tokenMixConfig = {
  share: { color: "green", label: "Token share" }
} satisfies ChartConfig;
...
export function TokenMixChart({ composition }: { composition: TokenComposition }): React.JSX.Element {
  const rows = [
  ...
  return (
    <div className="atlas-token-mix grid gap-5">
      {total > 0 ? (
        <div aria-label="Token mix by share" className="h-[250px]" role="img">
          <RadarChart
            bloom="low"
            config={tokenMixConfig}
            data={chartData}
            margins={{ bottom: 24, left: 42, right: 42, top: 24 }}
            nameKey="category"
          >
            <Radar dataKey="share" variant="gradient" />
            <Tooltip
              valueFormatter={(value, _name, index) =>
                `${Math.round(value)}% ? ${formatTokens(chartData[index]?.tokens ?? 0)}`
              }
              variant="frosted-glass"
            />
          </RadarChart>
